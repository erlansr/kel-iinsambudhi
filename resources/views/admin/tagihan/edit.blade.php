...
<div class="bg-white rounded-lg shadow-md p-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">✏️ Edit Tagihan</h1>
        <a href="{{ route('admin.tagihan') }}" class="text-gray-600 hover:text-gray-800">← Kembali</a>
    </div>
    
    <form method="POST" action="{{ route('admin.tagihan.update', $pembayaran) }}">
        @csrf
        @method('PUT')
        ...
        <div class="grid gap-6">
            <div>
                ...
                <select name="keluarga_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg" required>
                    @foreach($keluarga as $k)
                        <option value="{{ $k->id }}" {{ old('keluarga_id', $pembayaran->keluarga_id) == $k->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="text" name="bulan" value="{{ old('bulan', $pembayaran->bulan) }}" 
            ...
            <div>
                ...
                <input type="number" name="nominal" value="{{ old('nominal', $pembayaran->nominal) }}" 
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg"
                       required>
                @error('nominal')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
